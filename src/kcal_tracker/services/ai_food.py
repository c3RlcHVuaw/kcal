  ...
  "foods": [
    {
      ...
      "packaged": true,
      "visible_brand": "бренд, если он прямо читается на упаковке, иначе null",
      "visible_label_text": "короткий читаемый текст с упаковки: бренд, вкус, линейка, вес, иначе null"
    }
  ]
}
...
- Если бренд или название упаковки не читаются уверенно, не выдумывай бренд: назови позицию
  "упакованный продукт, бренд не читается", поставь packaged=true и confidence не выше 0.45.
- Если на упаковке виден бренд, но часть вкуса/линейки не читается, используй только видимый бренд и тип продукта,
  а не похожий продукт из памяти.
...
def photo_recognition_user_text(text_hint: str | None = None, *, photo_count: int = 1) -> str:
    base = (
        ...
        "Для упаковки заполни visible_brand и visible_label_text только тем, что реально читается."
    )
    if photo_count > 1:
